...
public:
   ...
   DS_Factory();
   // According to the rules of C++, this should be a virtual destructor since the derived 
   // types DSBASE_Factory, DSCursor_Factory, DATAPACKETREAD_Factory, DATAPACKETWRITE_Factory
   // are deleted via a pointer to DS_Factory. However, none of the derived types have
   // a destructor. So, we will violate the C++ rule and venture in "Undefined Behaviour" territory.
   ~DS_Factory();
...
#else
   {
      // The call to 'delete DS_Factory' above makes newer versions of clang generate a
      // "-Wdelete-non-virtual-dtor" warning and emit 'ud2' opcodes here to highlight
      // the fact that invoking a non-virtual dtor on a polymorphic type is 
      // 'undefined behaviour' per the C++ standard.
      // So, we will free the memory but not make any attempt to invoke the destructor.
      void *p = static_cast<void*>(ppvObj);
      free(p);
   }
#endif
Exit:
...
DS_Factory::~DS_Factory()
{
  // This destructor must not be invoked when building w/ clang
  // CLANG will generate "ud2" instructions to highlight
  // the fact that we are in "Undefined Behaviour" 
  // See comment in DS_Factory class for more info.
   return;
}
...
#else
   {
      // The call to 'delete this' above makes newer versions of clang generate a
      // "-Wdelete-non-virtual-dtor" warning and emit 'ud2' opcodes here to highlight
      // the fact that invoking a non-virtual dtor on a polymorphic type is 
      // 'undefined behaviour' per the C++ standard.
      // So, we will free the memory but not make any attempt to invoke the destructor.
