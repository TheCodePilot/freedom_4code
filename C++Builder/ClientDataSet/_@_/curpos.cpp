...
DBIResult DBIFN DSCursor::PutField(pBYTE pRecBuf, UINT32 iFieldNo, pBYTE pFldBuf)
{
   ...
   if (pFldBuf)
   {
      ...
      else if (pDsFldDes->iFldType == fldUNICODE)
      { // trim blanks, pad with zeros (ASM)
         UINT32 iLen = pDsFldDes->iFldLen;
         pBYTE pDest = pRecBuf + pDsFldDes->iFldOffsInRec;
         ...
         if (pDs->bDisableStringTrimming == FALSE)
         {
            while (iLen && ((UINT16*)pFldBuf)[iLen/2] == (UINT16)' ')
               iLen -= 2;
         }
         *(UINT16*)pDest = iLen;
         if (iLen)
         {
            CopyMem(&pDest[sizeof(UINT16)], &pFldBuf[sizeof(UINT16)], iLen);
         }
      }
...
DBIResult DBIFN DSCursor::MoveToBookMark(pBYTE pBookMark)
{
   ...
   { 
      ...
      if (! iCurPos && pDs->pDsLog)
      {
         // if not found, then look for newer record versions (RS-18584)
         UINT32 iRecNo = pBk->iRecNo;
         do
         {
            iRecNo = pDs->pDsLog->GetNextRecNo(iRecNo);
         }
         while (iRecNo && ! (iCurPos = pIdx->IdxRecNoToSeqNo(iRecNo)));
      }
   }
   else
      iCurPos = pBk->iPos;
