   ...
   if (pDs->iMDSemantics & mdMASTER && pDs->pLinkedTables)
   {
      ...
      while ((pEntry = pDs->pLinkedTables->GetEntry(i++)) != NULL)
      {
         ...
         if (phLinkCur)
         {
            ...
            {
               bHasDetails = TRUE;
               bRemoveDetails = (rslt != DBIERR_DETAILSNOTFETCHED) && iRecCnt;
            }
         }
         else
         {
         ...
         if (bHasDetails)
         {
            ...
            // Cascade deletes
            // * if there is delayed fetching, we might not want to allow this!,
            //   since we might not have all details
            // * but if we will not do that, then DBIERR_DATAPACKETMISMATCH may
            //   be returned from RECONCB::Init - see RS-92421
            if (bRemoveDetails)
            {
               phLinkCur->MoveToBOF();
               for (j = 0; j < iRecCnt; j++)
               {
                  rslt = phLinkCur->MoveRelative(1);
                  if (rslt)
                     break;
                  rslt = phLinkCur->DeleteRecord();
                  if (rslt)
                  {
                     // Undo
                     while (j--)
                        phLinkCur->UndoLastChange(FALSE);
                     goto Exit;
                  }
               }
            }
