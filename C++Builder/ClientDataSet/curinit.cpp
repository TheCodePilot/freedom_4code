...
DBIResult DBIFN DSCursor::UseIndexOrder(pCHAR pszName)
{
   ...
   else
   {
      if (pIdxNew) // Found one
      {
         ...
         if (pIdx->pGroups)
            pIdx->pGroups->Clone(pIdxNew, &pIdxNew->pGroups);
...
DBIResult DBIFN DSCursor::SortOnFields(UINT32 iFields, pULONGP piFields, pBOOL pbDescending, pBOOL pbCaseInsensitive)
{
   ...
   DSLOCK(pDs);
   if (iFields == 0 || piFields == NULL || iFields > DBIMAXFLDSINKEY)
   {
...
DBIResult DBIFN DSCursor::GetAggregateValue(hDSAggregate hAgg, pBYTE pValue, pBOOL pbBlank)
{
   ...
   { // Global aggregate
      ...
      if (pIdx->pGroups)
      {
         rslt = pIdx->pGroups->GetAggValue(hAgg, iCurPosAdj, iCurFirst, iCurLast, pValue, pbBlank);
         if (rslt == DBIERR_NONE)
            goto Exit;
      }
      pIdxTmp = pDs->pIdx;
      ...
      { // Try to find it on all other indexes
         if (pIdxTmp->pGroups)
            rslt = pIdxTmp->pGroups->GetAggValue(hAgg, iCurPosAdj, iCurFirst, iCurLast, pValue, pbBlank);
         if (rslt == DBIERR_NONE)
            goto Exit;
   ...
   if (!iCurState)
   {
      ...
      rslt = pIdx->pGroups->GetAggValue(hAgg, iCurPos, iCurFirst, iCurLast, pValue, pbBlank);
   }
   else
