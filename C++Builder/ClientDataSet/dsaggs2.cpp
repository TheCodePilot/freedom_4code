...
DBIResult DBIFN IDXGROUPING::GetAggValue(hDSAggregate hAgg, UINT32 iPos, UINT32 iPosMin, UINT32 iPosMax, pBYTE pValue, pBOOL pbBlank)
{
   DBIResult rslt = DBIERR_NONE;
   ...
   { // Needs a recalc
      ...
      if (rslt == DBIERR_NONE)
      {
         ...
         RangeOfSubGroup(iPos, iPosMin, iPosMax, &iPosFirst, &iCnt);
...
DBIResult DBIFN IDXGROUPING::RangeOfSubGroup( // Determines first position for subgroup, and number of elements
   ...
   UINT32 iPosMin, // Minimal position value
   UINT32 iPosMax, // Maximal position value
   ...
   )
{
   ...
   if (piSBG)
   {
      ...
      // Find first pos
      i = iPosCurrent;
      while (i >= iPosMin && piSBG[i] == iSubGroupId)
      {
         i--;
      }
      iPosFirst = i+1;
      iCnt = 0;
      for (i = iPosFirst; i <= iPosMax; i++)
      {
         if (piSBG[i] == iSubGroupId)
         {
            iCnt++;
...
DBIResult DBIFN IDXGROUPINGMNG::GetAggValue(hDSAggregate hAgg, UINT32 iPos, UINT32 iPosMin, UINT32 iPosMax, pBYTE pValue, pBOOL pbBlank)
{
   DBIResult rslt = DBIERR_NONE;
   IDXGROUPING *pGr = GetGroupObj(hAgg);
   ...
   {
      rslt = pGr->GetAggValue(hAgg, iPos, iPosMin, iPosMax, pValue, pbBlank);
   }
   return rslt;
}
