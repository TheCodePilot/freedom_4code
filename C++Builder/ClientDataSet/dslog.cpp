...
{
   return CompactLog(pLogNew, FALSE);
}

DBIResult DSLOG::CompactLog(DSLOG *pLogNew, BOOL bUntilFirstChange)
{
      ...
      case dsDetUpd:
         { // iRecNo1 is Master-RecNo, iRecNo2 is iDetID
            ...
            if (!bIgnore)
            {
               ...
               bIgnore = (pDsDet->pDsLog->CompactLog(pDetLogNew, TRUE) == DBIERR_NONE) && (pDetLogNew->iLast == 0);
               delete pDetLogNew;
            }
   ...
   Next:
      if (bUntilFirstChange && pLogNew->iLast > 0)
         break;
   } // End for
...
// Returns RecNo of next version of 'iRecno', if any
UINT32 DSLOG::GetNextRecNo(UINT32 iRecNo)
{
   UINT32 i;

   for (i = 0; i < iLast; i++)
   {
      if (pLogEntries[i].iRecNo2 == iRecNo && pLogEntries[i].iAttr & dsRecModified)
      {
         return pLogEntries[i].iRecNo1;
      }
   }
   return 0;
}
