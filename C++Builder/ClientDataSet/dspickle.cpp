...
DBIResult DBIFN GetFldObj(UINT32 iFieldType, BOOL bConvert, class DSXML_W *pDsXml, FLDConv **ppFldObj)
{
   ...
   case dsfldEMBEDDEDTBL:
      if (pDsXml)
         *ppFldObj = (FLDConv*)new FLDCNested(bConvert);
      else
   ...
   case dsfldBYTES:
      if (pDsXml)
      {
         if (bVarying)
         {
            *ppFldObj = (FLDConv*)new FLDCBinary(iSize);
         }
   ...
   case dsfldUNICODE:
      if (pDsXml)
      {
         *ppFldObj = (FLDConv*)new FLDCUnicode(iSize);
         break;
      }
   case dsfldZSTRING:
...
FLDCUnicode::FLDCUnicode(UINT32 iSize)
{
...
// XmlOnly
pBYTE FLDCUnicode::FldPut(pBYTE pPos, pBYTE pVal, UINT32 iVarLen)
{
...
// Xml only
pBYTE FLDCUnicode::FldGet(pBYTE pStr, pBYTE pPos, pUINT32 piLen, pBOOL pbBlank)
{
...
FLDCBinary::FLDCBinary(UINT32 iSize)
{
   iLen = iSize; // prefix size : 1:2:4
   this->bConvert = FALSE;
}
...
// XmlOnly
pBYTE FLDCBinary::FldPut(pBYTE pPos, pBYTE pVal, UINT32 iVarLen)
{
...
// Xml only
pBYTE FLDCBinary::FldGet(pBYTE pStr, pBYTE pPos, pUINT32 piLen, pBOOL pbBlank)
{
   if (pPos)
   {
...
FLDCAlwaysNull::FLDCAlwaysNull()
{
}

FLDCAlwaysNull::FLDCAlwaysNull(BOOL bConvert)
{
   iLen = 0;
   this->bConvert = bConvert;
}
...
pBYTE FLDCAlwaysNull::FldPut(pBYTE pPos, pBYTE pVal, UINT32 iLen)
{
...
pBYTE FLDCAlwaysNull::FldGet(pBYTE pPos, pBYTE pVal, pUINT32 piLen, pBOOL pbBlank)
{
   UNREFERENCED_PARAMETER(pVal);
   if (piLen)
      *piLen = 0;
...
FLDCNested::FLDCNested()
{
}

FLDCNested::FLDCNested(BOOL bConvert)
{
   iLen = 0;
   this->bConvert = bConvert;
...
pBYTE FLDCNested::FldPut(pBYTE pPos, pBYTE pVal, UINT32 iIgnLen)
{
   if (pDsXml)
...
pBYTE FLDCNested::FldGet(pBYTE pVal, pBYTE pPos, pUINT32 piLen, pBOOL pbBlank)
{
   UNREFERENCED_PARAMETER(pPos);
   if (pDsXml)
   {
