...
QPatternMB::QPatternMB(pCHAR pat, UINT16 fldNum, CHAR escapeChar, BOOL bCase)
{
   ...
   /* estimate based on worst case patterns "_t_t_t" --> 5(x86)-7(x64) bytes per char */
   UINT16 patMatchCodeEstimation =
    (patLength / 2) * (sizeof(BYTE) + sizeof(UINT16)) +
    (patLength / 2 + 1) * (sizeof(BYTE) + sizeof(UINT16) + sizeof(pCHAR)) +
    2 * sizeof(BYTE);
   patMatchCode = new BYTE[patMatchCodeEstimation];
   ...
   *pCode = DONE;
   ...
   if (pCode - patMatchCode > patMatchCodeEstimation)
   {
