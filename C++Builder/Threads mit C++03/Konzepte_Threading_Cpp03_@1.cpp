...
Hier ist ein einfaches Beispiel für die Verwendung von Boost.Thread:
...
Falls du spezifische Fragen hast oder weitere Unterstützung benötigst, stehe ich dir gerne zur Verfügung!

Was kostet die Boost-Bibliothek ?
Die Boost-Bibliothek ist kostenlos. Sie steht unter der Boost Software License, die es erlaubt, die Bibliotheken kostenlos zu nutzen, zu verändern und zu verbreiten2. Du kannst die Bibliothek von der offiziellen Boost-Website herunterladen.
...
Ich arbeite für ein Programm was nur auf Windows-Rechner läuft. Wäre Boost oder dies hier" Windows API: Wenn du unter Windows arbeitest, kannst du die Windows API für Multithreading verwenden. Diese API bietet Funktionen wie CreateThread, WaitForSingleObject und EnterCriticalSection, die grundlegende Threading-Funktionalität bereitstellen." besser in diesem Fall ?
Da du ausschließlich auf Windows arbeitest, könnte die Verwendung der Windows API eine praktikable Option sein, insbesondere wenn du eine sehr plattform-spezifische Lösung benötigst. Die Windows API bietet dir direkten Zugriff auf das Betriebssystem und ermöglicht eine fein abgestimmte Steuerung der Threading-Funktionen. Hier sind einige Vorteile der Windows API:

Direkte Kontrolle: Du hast direkten Zugriff auf die Thread-Management-Funktionen des Betriebssystems, was maximale Flexibilität und Kontrolle bietet.

Leistung: Oftmals sind die nativen APIs des Betriebssystems schneller, da sie weniger Overhead haben und speziell auf die Hardware und das Betriebssystem abgestimmt sind.

Boost.Thread ist jedoch auch eine hervorragende Wahl, insbesondere wenn du in Erwägung ziehst, dass dein Projekt in der Zukunft portierbar sein könnte oder wenn du eine höhere Abstraktionsebene bevorzugst. Hier sind einige Vorteile von Boost.Thread:

Portabilität: Boost.Thread ist plattformunabhängig, was bedeutet, dass du in der Zukunft möglicherweise auf andere Betriebssysteme portieren kannst, ohne größere Änderungen am Threading-Code vornehmen zu müssen.

Moderne C++ API: Boost.Thread bietet eine moderne und intuitive API, die die Verwendung von Threads und Synchronisationsmechanismen vereinfacht.

Hier ist ein kurzes Beispiel für die Verwendung der Windows API, um einen einfachen Thread zu erstellen:
...
Ich brauche echte Atomare und Mutexe um gute Sicherheit zu bieten. Wer ist dann die beste Wahl ?
Wenn du echte atomare Operationen und Mutexe benötigst, um die Thread-Sicherheit zu gewährleisten, ist die Verwendung der Windows API oder Boost.Thread jeweils gute Optionen, die für unterschiedliche Anwendungsfälle besser geeignet sein könnten:

Boost.Thread:

Portabilität: Ideal, wenn du planst, dein Projekt möglicherweise auch auf andere Betriebssysteme zu portieren.

Moderne C++ API: Bietet eine intuitive API und unterstützt moderne C++-Funktionalitäten.

Thread-Sicherheit: Bietet Unterstützung für atomare Operationen und Mutexe.
...
Direkte Kontrolle: Bietet direkten Zugriff auf die Thread-Management-Funktionen des Betriebssystems.

Leistung: Kann in einigen Fällen schneller sein, da es weniger Overhead hat.
...
DWORD WINAPI ThreadFunc(void* data)
{
	...
	if (form)
	{
		...
		form->Synchronize(&form->asLamdaShowThreadMessage);
	}
	Sleep(1000);
	return 0;
}
...
void TForm5::asLamdaShowThreadMessage()
{
	ShowMessage("std::cout << Hello from thread! << std::endl;");
}
