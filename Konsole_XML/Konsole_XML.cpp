...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	BSTR dataINodeNamebstrName;
	BSTR dataINodeNameDate;
	BSTR dataINodeName;
	BSTR dataINodeValue;
	BSTR dataHelp;
	BSTR dataHelp1;
										...
										if (SUCCEEDED(hr) && wcscmp(dataINodeValue, L"21") == 0) {
											...
											if (SUCCEEDED(hr))
											{
												...
												if (SUCCEEDED(hr) && wcscmp(dataINodeName, L"bstrName") == 0)
												{
													...
													if (SUCCEEDED(hr) && wcscmp(dataINodeValue, L"C1 (f)") == 0)   //dataINodeNamebstrName;    dataINodeNameDate;
																						...
																						while (pChildWithInterestingDatas_Results_Item->nextNode(&pChildWithInterestingData_Results_Item) == S_OK) {
																							...
																							while (pChildWithInterestingDatas_Results_Item_Childs->nextNode(&pChildWithInterestingData_Results_Item_Childs) == S_OK) {
																								...
																								while (pChildWithInterestingDatas_Results_Item_Childs_Describer->nextNode(&pChildWithInterestingData_Results_Item_Childs_Describer) == S_OK){
																									...
																									//hr = pChildWithInterestingData->get_nodeName(&dataINodeName); //wäre Wert
																									hr = pChildWithInterestingData_Results_Item_Childs_Describer->get_nodeName(&dataINodeName);    //get_nodeValue(pwert);     //(&dataINodeName); //wäre Wert
																									...
																									if (( SUCCEEDED(hr)&&(wcscmp(dataINodeName, L"dValue")) == 0)&&(wcscmp(dataHelp, L"Vx") == 0) )       //RRR6
																									{
																												...
																												hr = pChildWithInterestingData_Results_Item_Childs_Describer->get_childNodes(&pChildsTest);
                                                                                                                if (FAILED(hr)) {
																												...
																												while (pChildsTest->nextNode(&pChildTest) == S_OK){
																													...
																													std::wcout << "Name der Messgtroesse: " << dataHelp << std::endl;
																													std::wcout << "Einheit__________Alle Kinder von dem jeweiligen Daten  __ Einheit: " << dataINodeName << std::endl;
																													std::wcout << "VALUE__________Alle Kinder von dem jeweiligen Daten  __ Wert: " << dataHelp1 << std::endl;

																													pChildTest->Release();
																												}
																												pChildsTest->Release();

																													//std::wcout << "__________Alle Kinder von dem jeweiligen Daten  __ Knotenname: " << " Name: " << pwert << std::endl;
																													...
																													//break;     //RRR6 aber wieder rein nach Test
																									}
