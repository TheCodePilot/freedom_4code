...
//class 4 XML-Handling
	...
	public:
		...
		void ProcessNodePlus(IXMLDOMNode* pNode);
            void ProcessNodePlusSpezial(IXMLDOMNode* pNode);
		...
		bool findstart;
        int  tagCounter;
		void getNodeInfoByTagName(IXMLDOMNode* pNode, const std::wstring& tagName);
		void processNodePlus(IXMLDOMNode* pNode);   //von RRTools
		void processNode(IXMLDOMNode* pNode);       //von RRTools

		void getNodeInfoByTagNameNativ(IXMLDOMNode* pNode, const std::wstring& tagName);
		void processNodeNativ(IXMLDOMNode* pNode);

	private:
		void ReleaseResources();
		bool AcceptValues(IXMLDOMNode* pNode);
	...
	class Pair2Strings
	{
		public:
			...
			std::pair<std::wstring, std::wstring> GetThePair(int Index)const;
			void SetThePair(std::pair < std::wstring, std::wstring >  xmlNodePairIn); //einfach nexte füllen ohne Index
		private:
			std::vector < std::pair < std::wstring, std::wstring > > xmlNodePair;
