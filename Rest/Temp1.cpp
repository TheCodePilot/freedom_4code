...
//	TFormXML_ (this);
	TFormXML_ (MyFrame);
		//MyFrame->NgDB->DataSource = DS_Dummy;
			//MyFrame->NgDB->DataSource->DataSet->DisableControls();
		   /*	MyFrame->NgDB->DataSource = DataSource3;
			MyFrame->NgDB->DataSource->DataSet->DisableControls();
			MyFrame->NgDB->DataSource->DataSet = CDS_Dummy;
			MyFrame->NgDB->DataSource->DataSet->DisableControls();*/
			//MyFrame->Ichframe(); // OK
...
void __fastcall TFormXML::FirstRowHead()
{

	MyFrame->Ng->BeginUpdate();
	MyFrame->Ng->ClearRows();
	MyFrame->Ng->AddRow(1);
	int rowIndex = 0;

	for(int i = 0; i < NSIZEOF_CONTAINER; i++)
	{
		MyFrame->Ng->Columns->Add(__classid(TNxTextColumn)); //, System::WideString(""));
		//MyFrame->Ng->Columns->Add(__classid(TNxTextColumn), true);
		MyFrame->Ng->Cell[i][rowIndex]->AsString = "NEU" + IntToStr(i+1) + "1";
	}

	MyFrame->Ng->EndUpdate();
}

void __fastcall TFormXML::AddOtherRows(int rowIndex)
{
	...
	for(int i = 0; i < NSIZEOF_CONTAINER; i++)
	{
		MyFrame->Ng->Cell[i][rowIndex]->AsString = "A" + IntToStr(i+1) + "1";
		MyFrame->Ng->Cell[i][rowIndex]->AsString = "A" + IntToStr(i+1) + "2";
		MyFrame->Ng->Cell[i][rowIndex]->AsString = "A" + IntToStr(i+1) + "3";
	}
	MyFrame->Ng->EndUpdate();
}

//Hier noch nicht geändert
void __fastcall TFormXML::Transfer2DB_C1()
{

	MyFrame->NgDB->BeginUpdate();
	DataSource4->DataSet = ClientDataSet4;
	MyFrame->NgDB->DataSource = DataSource4;
	MyFrame->NgDB->ReadOnly = true;
	...
	MyFrame->NgDB->EndUpdate();
//SendFormate2Grid();

	/*// Setze die Überschriften der Spalten fett darstellen
	for (int i = 0; i < MyFrame->NgDB->Columns->Count; ++i) {
		MyFrame->NgDB->Columns->Item[i]->Font->Style = TFontStyles() << fsBold;// Title->Font->Style = TFontStyles() << fsBold;
	}
	MyFrame->NgDB->Repaint();*/
...
void __fastcall TFormXML::Transfer2DB_MA()
{
	...
	MyFrame->NgDB1->BeginUpdate();
//DataSource5->DataSet = ClientDataSet5;
	//------------NEU
	MyFrame->DS->DataSet = ClientDataSet5;
	//------------NEU
//MyFrame->NgDB1->DataSource = DataSource5;
	MyFrame->NgDB1->ReadOnly = true;
...
//Hier rest automatisch von der Datenbank wenn noch nicht belegt
//abfragen was Frame gemacht hat und sagen was der User hier alles gemacht hat im XML-Form
void __fastcall TFormXML::OnClose(TObject *Sender, TCloseAction &Action)
{
	ShowMessage("OnClose");
}
//---------------------------------------------------------------------------

void __fastcall TFormXML::SendFormate2Grid()
{
//MyFrame->NgDB->Color = clSkyBlue;	// Setze die Überschriften der Spalten fett darstellen
	//MyFrame->NgDB->FirstRow = 0;
	for (int i = 0; i < MyFrame->NgDB->Columns->Count; ++i) {
		//MyFrame->NgDB->Columns->Item[i]->Field->SetFieldProps() Font->Style = TFontStyles() << fsBold;// Title->Font->Style = TFontStyles() << fsBold;
			   // ->Item[i]->Font->Style = TFontStyles() << fsBold;
	}

   /*	for (int row = 0; row < MyFrame->NgDB->RowCount; ++row) {
		for (int col = 0; col < MyFrame->NgDB->Columns->Count; ++col) {
			MyFrame->NgDB->Cells[col][row]->Color = clSkyBlue; //clYellow; // Hier die gewünschte Hintergrundfarbe einsetzen
		}
	}
	*/
	MyFrame->NgDB->Repaint();
		//for(int i = 0; i < MyFrame->NgDB->Columns->Count; ++i)
			//MyFrame->NgDB->Columns->Item[i]-
	//MyFrame->NgDB->TitleFont->Style = MyFrame->NgDB->TitleFont->Style << fsBold;
}

//@ Farbe wird wie gesteuert"ShowMessage("OnDestroy")
