...
{
	ReleaseResources();
	//ShowMessage("Destruktor Parent: XMLBasic"); RRR42
}


 void RRClasses::XMLBasic::ReleaseResources()
 {
			if (pXMLDom) {
				pXMLDom->Release();
				pXMLDom = NULL;
 ...
 VARIANT_BOOL RRClasses::XMLBasic::LoadXMLDocument(const std::wstring& filename) {
...
RRClasses::XMLData::XMLData()	//: XMLProcess()
{
	pXMLDom = NULL;
	isInitializedCOM = false;
...
RRClasses::XMLData::~XMLData()
{
	...
	ClearTheInfo();
	ReleaseResources();
	//ShowMessage("Destruktor Child: XMLData"); RRR42
	//std::cout << "Desruktor:  XMLData" << std::endl;
}

//Bei exception alles aufräumen
void RRClasses::XMLData::ReleaseResources()
{
	if (pRootNode) {
		pRootNode->Release();
		pRootNode = NULL;
	...
	RRClasses::XMLBasic::ReleaseResources();
	//ReleaseResources;
}

void RRClasses::XMLData::ClearTheNodes()
{
...
void RRClasses::XMLData::ClearTheInfo()
{
	xmlInfo.clear();
	xmlInfo4UserChoice.clear();
}


void RRClasses::XMLData::ClearTheInfo4GUI()
{
...
void RRClasses::XMLData::CreateGuiNode(IXMLDOMNode* node)
{
	BasicNode bnode;
...
//=============================================================================================================================================
IXMLDOMNode* RRClasses::XMLData::GetNode(size_t index)const {
...
HRESULT RRClasses::XMLData::IniRoot()
{
...
void RRClasses::XMLData::WriteInfos()
{
	HRESULT hr;
	BSTR nodename = NULL;
	BSTR nodevalue = NULL;
...
std::pair<std::wstring, std::wstring> RRClasses::XMLData::GetNodeInfo(IXMLDOMNode* pNode) const {
...
int RRClasses::XMLData::FindTheMeasurementData(IXMLDOMNodeList* pSiblings, int option)	//02/26/2019 10:59:09
{
...
int RRClasses::XMLData::FindTheMeasurementDataStrCat(IXMLDOMNodeList* pSiblings, int option)
{
	BSTR dataINodeNameDate = NULL;
	BSTR dataINodeName = NULL;
	BSTR dataINodeValue = NULL;
 ...
 HRESULT RRClasses::XMLData::AnalyzeDOM()
{
	 HRESULT hr = IniRoot();
	 if (FAILED(hr))
		 return hr;
...
int RRClasses::XMLData::AnalyzeDOM_PicAllEvents() //RRR 40
{
	int errornumber = 0;
...
int RRClasses::XMLData::AnalyzeDOM_ProofTyp(size_t coiceInList) //RRR 40
{
	int errornumber = 0;
			...
			while (pChildrenChildrenOfCards->nextNode(&pChildChildNodeOfCards) == S_OK)
			{
				...
				if (pSiblingNode && SUCCEEDED(hr))
				{
					...
					if (pSiblingNode && SUCCEEDED(hr) && wcscmp(dataINodeName, L"nType") == 0)
					{
						...
						else
						{
							...
							while (pChildrenChildrenOfCards->nextNode(&pChildChildNodeOfCards) == S_OK)
							{
								...
								if (pSiblingNode && SUCCEEDED(hr))
								{
									dataINodeName = NULL;
									hr = pSiblingNode->get_nodeName(&dataINodeName);
									if (dataINodeName && SUCCEEDED(hr) && wcscmp(dataINodeName, L"bstrComment") == 0)
									{
										hr = pSiblingNode->get_text(&dataINodeName); //hier der Wert also Text des Kommentars wie L"Ich bin ein Kommentar"
										if (dataINodeName && SUCCEEDED(hr))
										{
											commentIfType18 = dataINodeName;
											errornumber = -1;
										}
						...
						//-------------------------NEU
						//Fall wenn nichts gefunden wird bzw völlig anderer Typ Besprechung hier mit dem Kunden
						//was wenn anderer Typ
                        if(pChildChildNodeOfCards)
...
int RRClasses::XMLData::AnalyzeDOM_PicEventUwish(std::wstring ereignis, std::wstring datum,int option) //RRR 40
{
	ClearTheInfo();
	DummyIfThis();
...
//------------------------------
const std::vector<std::pair<std::wstring, std::wstring> >& RRClasses::XMLData::GetXMLInfo() const {
	return xmlInfo;
}

//Hier werden die Cards_Item eingesammelt
int RRClasses::XMLData::CollectCards_Items()
{
	int errornumber = 0;
